...
import ShowActionTask from './ShowActionTask'
import Modal from './Modal'
const InputTask = ({ isTask = true, id, isDone, content }) => {

    const { handleAddTask, handleUpdateTask, classDrag } = useContext(taskContext)
    ...
    const [taskAdded, setTaskAdded] = useState({ type: '', message: '' })
    ...
    useEffect(() => handleUpdateTask(id, isCheck, 'done', content), [isCheck])

    return (
        <>
            {modalTask && <Modal setModalTask={setModalTask} id={id} type='one' />}
            {isTask
                ?
                <>
                    <div className={`dark:bg-containerDark dark:text-textDark bg-containerLight text-textLight w-full group flex items-center transition-all duration-200 justify-between ${classDrag}  py-4`}>
                    ...
                    {modalInTask && <ShowActionTask type={taskAdded.type} message={taskAdded.message} />}
                </>
                :
                <>
                    <form className='dark:bg-containerDark dark:text-textDark bg-containerLight text-textLight w-full flex items-center justify-between rounded-md overflow-hidden transition-colors duration-300 mb-8' onSubmit={(e) => handleSubmit(e)}>
                        ...
                        <input onChange={(e) => handleChange(e)} type="text" className='dark:bg-containerDark dark:text-textDark bg-containerLight text-textLight w-[calc(100%-8rem)] py-4 px-3 text-xs outline-none transition-colors duration-300' value={task} placeholder='Create a new task...' />
                        <ButtonAddTask handleSubmit={handleSubmit} setTaskAdded={setTaskAdded} />
                    </form>
                    {modalInTask && <ShowActionTask type={taskAdded.type} message={taskAdded.message} />}
